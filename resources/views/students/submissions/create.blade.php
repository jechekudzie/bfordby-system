...
@section('submission_content')
    @if($allocation->is_timed && !$submission->start_time)
        ...
        <div class="alert bg-brand-secondary-light">
            <div class="d-flex">
                <div class="me-3">
                    <i class="fas fa-stopwatch fa-2x text-brand-primary"></i>
                </div>
                <div>
                    <h5 class="alert-heading text-brand-primary">Timed Assessment</h5>
            ...
            <div class="card-body">
                @if($group)
                    <div class="alert bg-brand-secondary-light">
                        <div class="d-flex">
                            ...
                            <div>
                                <h5 class="alert-heading text-brand-primary">Group Information</h5>
                    ...
                    <h6 class="mb-3 text-brand-primary">Group Members:</h6>
                    ...
                    <div class="d-grid gap-2">
                        @if($allocation->questions->isNotEmpty())
                            <a href="{{ route('students.submissions.show', ['allocation' => $allocation, 'group' => 1]) }}" class="btn btn-brand-primary btn-lg">
                                <i class="fas fa-users me-2"></i>Submit Group Assignment
                            </a>
                        @else
                            <a href="{{ route('students.submissions.types.upload', ['allocation' => $allocation, 'group' => 1]) }}" class="btn btn-brand-primary btn-lg">
                                <i class="fas fa-users me-2"></i>Submit Group Assignment
                            </a>
                        @endif
                    </div>
                @else
                    <div class="alert bg-brand-secondary-light">
                        <div class="d-flex">
                            <div class="me-3">
                                <i class="fas fa-exclamation-triangle fa-2x text-brand-primary"></i>
                            </div>
                            <div>
                                <h5 class="alert-heading text-brand-primary">Group Required</h5>
                    ...
                    <div class="d-grid gap-2">
                        <a href="{{ route('assessment-allocation-groups.index', $allocation) }}" class="btn btn-brand-primary btn-lg">
        ...
        <div class="card mb-4">
            <div class="card-header bg-light">
                <h6 class="mb-0 text-brand-primary">
                    <i class="fas fa-info-circle text-brand-primary me-2"></i>Assessment Instructions
                </h6>
            </div>
            <div class="card-body">
                <div class="p-3 bg-brand-primary-light rounded">
    ...
    @if($allocation->due_date)
        <div class="alert bg-brand-secondary-light mb-4">
            <div class="d-flex align-items-center">
                <i class="fas fa-calendar-alt text-brand-primary me-3 fa-lg"></i>
                <div>
        ...
        @if($allocation->submission_type !== 'group' || $group)
            <div class="d-grid gap-2">
                @if($allocation->submission_type === 'upload')
                    <a href="{{ route('students.submissions.types.upload', $allocation) }}" class="btn btn-brand-primary btn-lg">
                        <i class="fas fa-file-upload me-2"></i>Upload and Submit File
                    </a>
                @elseif($allocation->submission_type === 'online')
                    <a href="{{ route('students.submissions.show', $allocation) }}" class="btn btn-brand-primary btn-lg">
                        <i class="fas fa-edit me-2"></i>Take and Submit Online Assessment
                    </a>
                @elseif($allocation->submission_type === 'in-class')
                    <div class="alert bg-brand-secondary-light">
                        <i class="fas fa-chalkboard-teacher me-2 text-brand-primary"></i>
