...
@section('title', 'Edit Module Content')

@section('content')
<div class="container-fluid px-4">
    <h1 class="mt-4">Edit Content - {{ $content->title }}</h1>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.courses.subjects.modules.contents.update', [$subject->slug, $module, $content]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="3">{{ old('description', $content->description) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="content_type" name="content_type" required>
                        ...
                        <option value="video" {{ old('content_type', $content->content_type) == 'video' ? 'selected' : '' }}>Video File</option>
                        <option value="audio" {{ old('content_type', $content->content_type) == 'audio' ? 'selected' : '' }}>Audio File</option>
                        <option value="image" {{ old('content_type', $content->content_type) == 'image' ? 'selected' : '' }}>Image</option>
                        <option value="youtube" {{ old('content_type', $content->content_type) == 'youtube' ? 'selected' : '' }}>YouTube Video</option>
                        <option value="external_link" {{ old('content_type', $content->content_type) == 'external_link' ? 'selected' : '' }}>External Link</option>
                    </select>
                </div>
                ...
                <div class="mb-3 file-upload-section">
                    ...
                    <div class="mt-2 current-file-info">
                        <p class="mb-1"><strong>Current file:</strong> {{ basename($content->file_path) }}</p>
                        <div class="d-flex align-items-center gap-2">
                            <a href="{{ $content->getFileUrl() }}" target="_blank" class="btn btn-sm btn-info">
                ...
                <div class="mb-3 external-url-section">
                    ...
                    <input type="url" class="form-control" id="external_url" name="external_url" value="{{ old('external_url', $content->external_url) }}">
                    ...
                    <div class="mt-2">
                        <a href="{{ $content->external_url }}" target="_blank" class="btn btn-sm btn-info">
                            <i class="fas fa-external-link-alt me-1"></i> Visit Current URL
                        </a>
                    </div>
                    @endif
                </div>
                ...
                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input" id="is_required" name="is_required" value="1" {{ old('is_required', $content->is_required) ? 'checked' : '' }}>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateFormVisibility() {
            ...
            if (['pdf', 'video', 'audio', 'image'].includes(selectedValue)) {
                ...
                // Hide external URL section
                if (currentFileInfo && selectedValue !== '{{ $content->content_type }}') {
            ...
            // Add warning if changing types
            if (selectedValue !== '{{ $content->content_type }}') {
                if (!document.getElementById('type-change-warning')) {
                    const warningDiv = document.createElement('div');
                    warningDiv.id = 'type-change-warning';
                    warningDiv.className = 'alert alert-warning mt-3';
                    warningDiv.textContent = 'Warning: Changing content type may require you to provide a new file or URL.';
                    contentTypeSelect.parentNode.appendChild(warningDiv);
                } else {
                    document.getElementById('type-change-warning').style.display = 'block';
                }
            } else if (document.getElementById('type-change-warning')) {
                document.getElementById('type-change-warning').style.display = 'none';
            }
        }
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (['youtube', 'external_link'].includes(selectedValue) && !externalUrlInput.value) {
                e.preventDefault();
                alert('Please enter a valid URL for ' + selectedValue + ' content.');
            }
            
            if (['pdf', 'video', 'audio', 'image'].includes(selectedValue) && 
                !['pdf', 'video', 'audio', 'image'].includes('{{ $content->content_type }}') && 
