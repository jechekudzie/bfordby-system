...
class PermissionController extends Controller
{
    ...
    {
        $modules = PermissionModule::all()->sortBy('id');
    ...
    public function store(Request $request)
    {
        if ($request->permission_type === 'module') {
            ...
            return redirect()->route('admin.permissions.index')
                           ->with('success', 'PermissionModule and default permissions created successfully');
        } else {
            ...
            $module = PermissionModule::findOrFail($request->module_id);
    ...
    public function destroy($id)
    {
        ...
        $module = PermissionModule::where('slug', explode('-', $permission->name)[1])->first();
        if ($module) {
            $defaultActions = ['view', 'create', 'read', 'update', 'delete'];
